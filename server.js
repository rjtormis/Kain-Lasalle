...
const dbConfig = {
  registration: {
    url: 'mongodb+srv://samuel:tam101301@cluster0.vtced9g.mongodb.net/',
    dbName: 'registration',
    collectionName: 'reguser',
  },
  aduser: {
    url: 'mongodb+srv://samuel:tam101301@cluster0.vtced9g.mongodb.net/administrator',
    dbName: 'administrator',
    collectionName: 'aduser',
  },
  // ... (other collections)
};
...
async function connectToDatabases() {
  try {
    for (const collectionKey in dbConfig) {
      ...
      const client = await MongoClient.connect(collectionConfig.url, { useNewUrlParser: true, useUnifiedTopology: true });
...
app.post('/register', async (req, res) => {
  try {
    ...
    if (existingUser) {
      res.status(200).json({ success: false, message: 'Username already exists. Please choose a different one.' });
      return;
    }

    const result = await registrationCollection.insertOne({ registrationType, schoolEmail, fullName, username, password });
    res.status(200).json({ success: true, message: 'Registration successful! Redirecting to login', userId: result.insertedId });
  } catch (err) {
